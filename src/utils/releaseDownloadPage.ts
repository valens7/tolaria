const RELEASE_HISTORY_URL = 'https://github.com/valens7/tolaria/releases'
const DOWNLOAD_FRAME_NAME = 'mora-download-frame'
...
function buildStableDownloadPageContent(
  ...
  if (Object.keys(downloads).length > 0) {
    return {
      ...
      message: 'Preparing the latest stable Mora download for your platform.',
      shouldRedirect: true,
      title: 'Mora Stable Download',
    }
  }

  return {
    helperText: 'Use the button below to check the latest release history.',
    message: 'No stable Mora downloads are available yet.',
    shouldRedirect: false,
    title: 'Mora Stable Download Unavailable',
  }
}
...
function buildDownloadFrameMarkup(downloads: StableDownloadTargets): string {
  ...
  return `<iframe title="Mora installer download" name="${DOWNLOAD_FRAME_NAME}" sandbox="allow-downloads" hidden></iframe>`
}
    ...
    <script>
      ...
      function downloadMessage(target, requiresMacChoice, requiresWindowsChoice) {
        ...
        if (requiresWindowsChoice) {
          return 'Use the signed Windows installer link below. Company-managed devices may require IT approval of the Mora publisher.';
        }

        return 'Starting the latest stable Mora download for ' + target.label + '.';
      }
